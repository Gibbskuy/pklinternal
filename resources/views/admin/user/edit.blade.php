            ...
            <form class="row g-3" method="POST" action="{{ route('user.update', $user->id) }}">
                ...
                <div class="col-md-8">
                    <div class="d-md-flex d-grid align-items-center gap-1">
                        <button type="submit" class="btn btn-grd-primary" class="text-white">Masukan</button> |
                        <button type="submit" class="btn btn-grd-primary"><a href="{{route('user.index')}}" class="text-white">Kembali</a></button>
                    </div>
                </div>
            </form>
        </div>
